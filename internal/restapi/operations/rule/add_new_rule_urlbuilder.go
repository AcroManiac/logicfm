...
func (o *AddNewRuleURL) Build() (*url.URL, error) {
	...
	var _path = "/rule/add/{musrvId}"

	musrvID := swag.FormatInt64(o.MusrvID)
	if musrvID != "" {
		_path = strings.Replace(_path, "{musrvId}", musrvID, -1)
	} else {
		return nil, errors.New("musrvId is required on AddNewRuleURL")
	}
...
func (o *AddNewRuleURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on AddNewRuleURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on AddNewRuleURL")
	}
